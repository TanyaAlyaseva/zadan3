globals [time  kol maxim rabtime l i j bol]
breed [komps komp]
breed [ps p]
...
;  reset-timer
 ...
 ; set who 0
  ...
  set time random-poisson 10
  set rabtime random-poisson 60
  set kol random-poisson 3
 ; if kolvo_l > kolvo_k [ask ps with [who  = kolvo_k ] [layout-circle ps 8]]
 ;plot count turtles
set l random (kolvo_k + kolvo_l - 1)
 ;user-message (l)
 ...
 ; user-message (timer)
  ...
  if kol > 5 [create-ps 1 [setxy -15 random-ycor set shape "person"   set label who set bol who
      set label-color red set j j + 1; user-message (j) user-message (who)
    ]
    ask ps with [who = bol - 4 ][layout-circle ps 8]]
 ; user-message (i)
